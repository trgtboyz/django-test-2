...
class ListContacts(APIView):
    ...
    def get(self, request, format=None):
        contacts = ContactDetails.objects.values()
...
def contact(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            vchr_name = contact_name,
            vchr_email = contact_email,
            txt_content = form_content
            )
...
def home(request):
    return render(request,'home.html',{'recipes': lst_recipes})

def contact_admin(request):
    queryset = ContactDetails.objects.order_by('-pk')
    serializer = ContactSerializer(queryset, many=True)
    return render(request,'contact-admin.html', {'data':serializer.data})
